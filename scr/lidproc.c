...
static double getStorageExfilRate(double storageDepth);                                       //(5.1.011)
...
//           infil = infiltration rate for LID unit (ft/s)
...
//
{
    ...
    double LID2GW     = theLidProc->storage.GWDepth - soilThickness - storageThickness;
    double LID2GW_ABS = abs(LID2GW);
    double GW2UD      = LID2GW + theLidProc->drain.offset;
    double GW2UD_ABS  = abs(GW2UD);
	...
	StorageExfil = getStorageExfilRate(storageDepth);

    //... underdrain flow rate
    StorageDrain = 0.0;
    if ( theLidProc->drain.coeff > 0.0 )
    {
        StorageDrain = getStorageDrainRate(storageDepth, soilTheta, 0.0, surfaceDepth);
    }

    //... identify water head inside
    head += storageDepth;
    if ( storageDepth >= storageThickness )
    {
        if ( soilThickness > 0.0 && soilTheta > soilFieldCap )
        {
            head += (soilTheta - soilFieldCap) / (soilPorosity - soilFieldCap) * soilThickness;
	...
	//... if groundwater is higher than LID bottom
	if (LID2GW <= 0.0)
	{
	    //... no bottom exfiltration at this condition
	    StorageExfil = 0.0;
        ...
        if ( storageThickness == 0.0 )
        {
            ...
            maxRate = (soilPorosity - soilTheta) * soilThickness / Tstep + SoilPerc + SoilEvap;
            SurfaceInfil = MIN(SurfaceInfil, maxRate);
        }
        //... if both soil and storage layers are full
        else if ( LID2GW_ABS >= soilThickness + storageThickness || ( soilTheta >= soilPorosity && storageDepth >= storageThickness ) )
        {
            maxRate = StorageExfil + StorageDrain;
            SoilPerc = MIN(SoilPerc, maxRate);
            SurfaceInfil = MIN(SurfaceInfil, maxRate);
        }
        //... if only storage layer is full
        else if ( LID2GW_ABS >= storageThickness || storageDepth >= storageThickness )
        {
            maxRate = StorageExfil + StorageDrain;
            SoilPerc = MIN(SoilPerc, maxRate);
            maxRate = (soilPorosity - soilTheta) * soilThickness / Tstep + SoilPerc + SoilEvap;
        ...
        {
            //... if groundwater or water level is higher than underdrain pipe
            if ( GW2UD <= 0.0 || storageDepth >= theLidProc->drain.offset )
            {
            ...
            //... if groundwater is lower than underdrain pipe
            else
            {
                availVolume =  (soilTheta - soilFieldCap) * soilThickness;
                maxRate = SurfaceInfil - SoilEvap + MAX(availVolume, 0.0) / Tstep;
	...
	//... if groundwater table is lower than LID bottom
	else
    {
        //... limit perc rate by available water
        availVolume =  (soilTheta - soilFieldCap) * soilThickness;
        ...
        else if ( storageThickness > 0.0 )
        {
            ...
            if ( StorageDrain > 0.0 )
            {
                ...
                {
                    maxRate += (storageDepth - theLidProc->drain.offset) * storageVoidFrac/Tstep;
                }
                maxRate = MAX(maxRate, 0.0);
                StorageDrain = MIN(StorageDrain, maxRate);
            }
            ...
            maxRate = StorageExfil + StorageDrain + StorageEvap + (storageThickness - storageDepth) * storageVoidFrac/Tstep;
            SoilPerc = MIN(SoilPerc, maxRate);
...
//
{
    ...
    // Groundwater variables
    double LID2GW     = theLidProc->storage.GWDepth - storageThickness;
    double LID2GW_ABS = abs(LID2GW);
    double GW2UD      = LID2GW + theLidProc->drain.offset;
    double GW2UD_ABS  = abs(GW2UD);
    ...
    //... identify water head in LID
    head += storageDepth;
    if (storageDepth >= storageThickness)
    {
        head += surfaceDepth;
    }
    if (theLidProc->drain.offset > 0.0)
    {
        head -= theLidProc->drain.offset;
    }

    //... if groundwater table is higher than LID
    //... and is higher than storage depth
    if (LID2GW <= 0.0)
    {
        StorageExfil = 0.0;
        if ( GW2UD < 0.0 || storageDepth > theLidProc->drain.offset )
        ...
        {
            if ( GW2UD < 0.0 && GW2UD_ABS > head )
            {
                maxRate = StorageInflow - StorageExfil - StorageEvap + GW2UD_ABS * storageVoidFrac/Tstep;
            }
            else
            {
                maxRate = StorageInflow - StorageExfil - StorageEvap + ( storageDepth - theLidProc->drain.offset ) * storageVoidFrac/Tstep;
            }
            StorageDrain = MIN(StorageDrain, maxRate);
        }
    }

    //... if groundwater table is lower than LID
    else
    {
        //... limit storage exfiltration by available storage volume
        ...
        maxRate = (storageThickness - storageDepth)*storageVoidFrac/Tstep + StorageExfil + StorageEvap + StorageDrain;
        StorageInflow = MIN(StorageInflow, maxRate);
    }
...
//
{
    ...
    // Groundwater variables
    double LID2GW     = theLidProc->storage.GWDepth - paveThickness - soilThickness - storageThickness;
    double LID2GW_ABS = abs(LID2GW);
    double GW2UD      = LID2GW + theLidProc->drain.offset;
    double GW2UD_ABS  = abs(GW2UD);
    ...
    //... get ET rates
    availVolume = SoilVolume - soilWiltPoint * soilThickness;
    getEvapRates(SurfaceVolume, PaveVolume, availVolume, StorageVolume, pervFrac);
    ...
    if ( theLidProc->drain.coeff > 0.0 )
    {
        StorageDrain = getStorageDrainRate(storageDepth, soilTheta, paveDepth, surfaceDepth);
    }
	...
	{
	    StorageExfil = 0.0;
        //... if no soil layer and pavement & storage layers are full
        if ( soilThickness == 0.0 && ( (LID2GW_ABS >= paveThickness + storageThickness) || ( soilTheta >= soilPorosity && storageDepth >= storageThickness ) ) )
        {
            maxRate = StorageExfil + StorageDrain + StorageEvap;
            PavePerc = MIN(PavePerc, maxRate);
            SoilPerc = PavePerc;
        ...
        //... if pavement & soil & storage layers are all full
        else if ( (LID2GW_ABS >= paveThickness + soilThickness + storageThickness) || ( paveDepth >= paveThickness && soilTheta >= soilPorosity && storageDepth >= storageThickness ) )
        {
            maxRate = StorageExfil + StorageDrain;
            SoilPerc = MIN(SoilPerc, maxRate);
            PavePerc = SoilPerc;
        ...
        //... if storage & soil layers are full
        else if ( (LID2GW_ABS >= soilThickness + storageThickness) || ( soilTheta >= soilPorosity && storageDepth >= storageThickness ) )
        {
            maxRate = StorageExfil + StorageDrain;
            SoilPerc = MIN(SoilPerc, maxRate);
        ...
        //... if soil and pavement layers are full
        else if ( soilThickness > 0.0 && paveDepth >= paveThickness && soilTheta >= soilPorosity )
        {
            PavePerc = MIN(PavePerc, SoilPerc);
            SoilPerc = PavePerc;
            SurfaceInfil = MIN(SurfaceInfil,PavePerc);
        }
        //... if no adjoining layers are full
        else
        {
            //... limit underdrain flow by volume above drain offset
            if (StorageDrain > 0.0)
            {
                ...
                if ( GW2UD < 0.0 && GW2UD_ABS > head )
                {
                    if (LID2GW < 0.0 && LID2GW_ABS >= storageThickness )
                    {
                        maxRate += SoilPerc;
                    }
                    maxRate += GW2UD_ABS * storageVoidFrac/Tstep;
                }
                else
                {
                    if (storageDepth >= storageThickness )
                    ...
                    if (storageDepth >= theLidProc->drain.offset)
                    {
                        maxRate += (storageDepth - theLidProc->drain.offset) * storageVoidFrac/Tstep;
                    }
                }
            ...
            if ( LID2GW_ABS >= storageThickness || storageDepth >= storageThickness )
            {
                availVolume = 0.0;
            }
            else
            {
                if ( GW2UD < 0.0 && GW2UD_ABS > head )
                {
                    availVolume = (storageThickness - LID2GW_ABS) * storageVoidFrac;
                }
                else
                {
                    availVolume = (storageThickness - storageDepth) * storageVoidFrac;
	...
	//... if groundwater is lower than LID bottom
	else
    {
        //... no soil layer, pavement & storage layers are full
        if ( soilThickness == 0.0 && storageDepth >= storageThickness && paveDepth >= paveThickness )
        {
        ...
        else if ( soilThickness > 0 && storageDepth >= storageThickness && soilTheta >= soilPorosity && paveDepth >= paveThickness )
        {
        ...
        //... storage & soil layers are full
        else if ( soilThickness > 0.0 && storageDepth >= storageThickness && soilTheta >= soilPorosity )
        {
...
//
{
    double depth;            // depth of surface waterUCF(RAINDEPTH) in swale (ft)
...
double getStorageExfilRate(double storageDepth)                                                   //(5.1.011)
//
...
//
{
	...
	double ThetaLidBottom;
	double LID2GW     = theLidProc->storage.GWDepth - theLidProc->pavement.thickness - theLidProc->soil.thickness - theLidProc->storage.thickness;
	double LID2GW_ABS = abs(LID2GW);

	if (theLidProc->storage.kSat == 0.0) return 0.0;
	if (MaxNativeInfil == 0.0) return 0.0;

	// soil moisture at the groundwater bottom (kun)
	ThetaLidBottom = theLidProc->storage.Thetar + (theLidProc->storage.Thetas - theLidProc->storage.Thetar) / (pow((1 + pow(theLidProc->storage.VGalpha * LID2GW_ABS, theLidProc->storage.VGn)), 1 - 1 / theLidProc->storage.VGn));

	// new exfiltration rate considered rise of groundwater table (kun)
	if (LID2GW < 0.0)
	{
	    LidExfil = 0.0;
	}
	...
	{
	    LidExfil = theLidProc->storage.kSat * (theLidProc->storage.Thetas - ThetaLidBottom) / (theLidProc->storage.Thetas - theLidProc->storage.Thetai);
	}
	...
	LidExfil = LidExfil * (1.0 - clogFactor);

	//... limit infiltration rate by any groundwater-imposed limit
	return MIN(LidExfil, MaxNativeInfil);
}
...
//           minus the drain outlet offset.
{
	...
	// --- calculate depth between groundwater table and underdrain (kun)
	Lid2GW = theLidProc->storage.GWDepth - theLidProc->surface.thickness - theLidProc->pavement.thickness - theLidProc->soil.thickness - theLidProc->storage.thickness;
	GW2UD = ABS(Lid2GW + theLidProc->drain.offset);

    // ... compute drain outflow from underdrain flow equation in user units
    //     (head in inches or mm, flow rate in in/hr or mm/hr)
    if ( head > ZERO )
    {
		if (Lid2GW < 0 && GW2UD > head) // if groundwater table higher than storage depth (kun)
		{
			outflow = theLidProc->drain.coeff *
				pow(GW2UD, theLidProc->drain.expon);
