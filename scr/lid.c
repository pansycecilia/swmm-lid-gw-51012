...
//  lid_readNativeSoil       called by parseLine in input.c
//  lid_readSoil             called by parseLine in input.c
...
static void   lid_getstate(int i, double x[]);
static void   lid_setstate(int i, double x[]);

static void   write2modflow(TLidUnit* LidUnit, TSubcatch* Subcatch, FILE* out, double x[], int i, int j);
...
void lid_getState(int i, double x[])
{
	lid_getstate(i, x);
}
...
void lid_setState(int i, double x[])
{
	lid_setstate(i, x);
}
...
void lid_getstate(int i, double x[])
{
	...
	lidGroup = LidGroups[i];
	if (lidGroup == NULL) return 0.0;
	if (Subcatch[i].lidArea == 0.0) return 0.0;
	lidList = lidGroup->lidList;
	lidUnit = lidList->lidUnit;
	x[0] = lidUnit->surfaceDepth;
	x[1] = lidUnit->storageDepth;
	x[2] = lidUnit->paveDepth;
	x[3] = lidUnit->soilMoisture;
}
...
void lid_setstate(int i, double x[])
{
	...
	lidList = lidGroup->lidList;
	lidUnit = lidList->lidUnit;
	lidUnit->surfaceDepth = x[0];
	lidUnit->storageDepth = x[1];
	lidUnit->paveDepth = x[2];
	lidUnit->soilMoisture = x[3];
}
...
//
{
	...
	LidProcs[j].storage.VGalpha     = x[4];
	LidProcs[j].storage.VGn         = x[5];
	LidProcs[j].storage.Thetar      = x[6];
	LidProcs[j].storage.Thetas      = x[7];
	LidProcs[j].storage.Thetai      = x[8];
	LidProcs[j].storage.GWDepth     = x[9] * 1000 / UCF(RAINDEPTH);
    return 0;
}
